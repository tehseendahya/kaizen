              ...
              {courseData.units.map((unit) => (
                <div key={unit.id} className="mb-2">
                  ...
                  >
                    ...
                    <svg
                      className={`w-5 h-5 text-gray-500 transition-transform duration-200 ${expandedUnits.includes(unit.id) ? 'rotate-180' : ''}`}
                  ...
                  {expandedUnits.includes(unit.id) && (
                    <div className="ml-4 mt-2 space-y-1">
                      {unit.intro && (
                        <div className="p-2 text-xs text-gray-600 bg-gray-50 rounded mb-2">{unit.intro}</div>
                      )}
                      {unit.lessons.map((lesson) => (
                        <button
                          ...
                          className={`w-full text-left px-4 py-3 rounded-lg transition-all cursor-pointer ${
                            selectedLesson === lesson.id ? 'bg-blue-900 text-white' : 'text-gray-700 hover:bg-gray-50'
                          }`}
                        >
                          <div className="text-sm font-medium truncate">{lesson.title}</div>
                          <div className={`text-xs mt-1 truncate ${selectedLesson === lesson.id ? 'text-blue-100' : 'text-gray-500'}`}>
                            {lesson.description}
                          </div>
                        </button>
                      ))}
            ...
            {selectedLesson ? (
              ...
              <div>
                <h2 className="text-2xl font-semibold text-gray-900">
                  Welcome to {courseShort ?? courseData.title.split(':')[0]}
                </h2>
          ...
          <div className="flex-1 p-4 sm:p-6">
            {selectedLesson ? (
              ...
              <div className="w-full">
                ...
                <div className="mb-8">
                  <button
                    onClick={() => courseData.units[0]?.lessons[0] && handleLessonClick(courseData.units[0].lessons[0].id)}
